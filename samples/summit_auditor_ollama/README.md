...
> **Status: needs-verification (search verified).** Summit County runs the Tyler
> iasWorld "Public Access" platform but serves a single unified `mode=realprop`
> search instead of the usual per-mode pages. The connector routes every search
> mode to `commonsearch.aspx?mode=realprop` via the `mode_map` knob and renames the
> two POST fields the realprop form uses (`inpNumber` -> `inpNo`, `inpOwner` ->
> `inpOwner1`) via `form_field_overrides` — the same knobs as Lake. These were
> confirmed live: past the site's `Disclaimer.aspx` gate, an `inpOwner1` owner
> search returned results under `jur=000`. What remains is the **detail** datalet
> layout (a third variant shared with Lake), so deep detail extraction is pending a
> `LAKE` `DetailProfile`. See
> [`docs/OHIO_AUDITOR_EXPANSION.md`](../../docs/OHIO_AUDITOR_EXPANSION.md) for the
> open items. This sample exercises tool selection and the canonical contract end
> to end.
