...
LOGGER = logging.getLogger("samples.montgomery_auditor_ollama.client")
MONTGOMERY_TOOL_NAME = "montgomery_county_auditor_search"


def local_src_paths(root: Path) -> list[Path]:
    # The Montgomery auditor tool ships in the shared iasWorld platform package
...
def montgomery_server_params(root: Path, log_level: str) -> Any:
...
def default_prompt(
    *,
    ...
    if scenario == "address":
        prompt = (
            "I'm reviewing a Montgomery County, Ohio property. Can you look up the "
            f"county auditor record for {address} and return the assessment record?"
        )
    elif scenario == "owner":
        prompt = (
            "I'm checking Montgomery County, Ohio auditor records for an owner. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ask Ollama a natural Montgomery County auditor question and log whether "
    ...
    parser.add_argument(
        "--parcel-id",
        default="A01 00107 0001",
        help="Montgomery County parcel ID for the parcel scenario",
    )
    parser.add_argument(
        "--address",
        default="100 EXAMPLE AVE",
        help="Montgomery County property address for the address scenario",
    )
    parser.add_argument(
        "--owner-name",
        default="DOE JANE A",
...
def main() -> None:
    ...
    final_text = asyncio.run(
        run_tool_trigger_sample(
            tool_name=MONTGOMERY_TOOL_NAME,
            prompt=prompt,
            model=args.model,
            ...
            logger=LOGGER,
            server_params_factory=montgomery_server_params,
        )
    )
    print(final_text)
