...
LOGGER = logging.getLogger("samples.stark_auditor_ollama.client")
STARK_TOOL_NAME = "stark_county_auditor_search"


def local_src_paths(root: Path) -> list[Path]:
    # The Stark auditor tool ships in the shared iasWorld platform package plus
...
def stark_server_params(root: Path, log_level: str) -> Any:
...
def default_prompt(
    *,
    ...
    if scenario == "address":
        prompt = (
            "I'm reviewing a Stark County, Ohio property. Can you look up the "
            f"county auditor record for {address} and return the assessment record?"
        )
    elif scenario == "owner":
        prompt = (
            "I'm checking Stark County, Ohio auditor records for an owner. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ask Ollama a natural Stark County auditor question and log whether "
    ...
    parser.add_argument(
        "--parcel-id",
        default="99000001",
        help="Stark County parcel ID for the parcel scenario",
    )
    parser.add_argument(
        "--address",
        default="100 EXAMPLE AVE NE",
        help="Stark County property address for the address scenario",
    )
    parser.add_argument(
        "--owner-name",
        default="DOE JANE A",
...
def main() -> None:
    ...
    final_text = asyncio.run(
        run_tool_trigger_sample(
            ...
            logger=LOGGER,
            server_params_factory=stark_server_params,
        )
    )
    print(final_text)
